...
export function getPendingCells(board) {
  const cells = [];
...
export function countPendingCells(board) {
...
export function validateLineShape(pendingCells) {
...
export function validateContiguous(board, pendingCells, orientation) {
...
export function validateFirstMove(pendingCells) {
...
export function isConnectedToBoard(board, pendingCells) {
...
export function validateConnected(board, pendingCells) {
...
function findWordThrough(board, row, col, orientation) {
  const cells = [];
  if (orientation === 'horizontal') {
    let start = col;
    ...
    for (let c = start; c <= end; c++) cells.push(getCell(board, row, c));
  } else {
    let start = row;
    while (hasLetter(getCell(board, start - 1, col))) start--;
    let end = row;
    while (hasLetter(getCell(board, end + 1, col))) end++;
    for (let r = start; r <= end; r++) cells.push(getCell(board, r, col));
  }
  return cells;
}

function toFormedWord(cells, orientation) {
  if (cells.length < 2) return null;
  return {
    word: cells.map((c) => c.letter).join(''),
    orientation,
    cells: cells.map((c) => ({ row: c.row, col: c.col, letter: c.letter, isNew: c.pending, bonus: c.bonus })),
  };
}
...
export function extractWords(board, pendingCells, orientation) {
  const words = [];
  const seen = new Set();
  const add = (fw, dir) => {
    if (!fw) return;
  ...
  if (orientation) {
    ...
    const crossDir = orientation === 'horizontal' ? 'vertical' : 'horizontal';
